...
public class CreationToolBar extends JToolBar {
    ...
    /**
     * counts the rectangles created so that each new rectangle gets
    ...
    private void addRectangle() {
        ...
        float cx = (float) visibleArea.getCenterX();
        float cy = (float) visibleArea.getCenterY();
        float d = (float) Math.min(visibleArea.getWidth(), visibleArea.getHeight()) / 2;
        GeoPath geoPath = new GeoPath();
        geoPath.append(GeoPath.newSquare(cx, cy, d), false);
        geoPath.setName("Rectangle " + this.rectangleCounter++);
        geoPath.setVectorSymbol(this.standardFillSymbol());
    ...
    private void addLine() {
        ...
        final float x1 = (float) (w / 4 + x);
        final float y1 = (float) (h / 4 + y);
        final float x2 = (float) (w / 4 * 3 + x);
        final float y2 = (float) (h / 4 * 3 + y);
    ...
    private void addCircle() {
        java.awt.geom.Rectangle2D visibleArea = this.mapComponent.getVisibleArea();
        final float w = (float) visibleArea.getWidth();
        final float h = (float) visibleArea.getHeight();
        final float r = Math.min(w, h) / 4.f;
        final float cx = (float) (visibleArea.getX() + w / 2);
        final float cy = (float) (visibleArea.getY() + h / 2);

        GeoPath geoPath = GeoPath.newCircle(cx, cy, r);
        geoPath.setName("Circle " + this.circleCounter++);
        geoPath.setVectorSymbol(this.standardFillSymbol());
        this.addGraphics(geoPath, true, "Circle");
    }
