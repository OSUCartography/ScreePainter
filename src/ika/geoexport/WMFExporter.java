...
public class WMFExporter extends VectorGraphicsExporter{
    ...
    throws IOException {
        ...
        short w = (short)this.dimToPagePx((float)this.pageFormat.getPageWidth());
        short h = (short)this.dimToPagePx((float)this.pageFormat.getPageHeight());
       ...
       do {
            switch (pi.getInstruction()) {
                ...
                case GeoPathModel.MOVETO:{
                    ...
                    polygon = new Polygon();
                    final int x = (short)this.xToPagePx(pi.getX());
                    final int y = (short)this.yToPagePx(pi.getY());
    ...
    private void writeGeoPoint(GeoPoint geoPoint) {
        ...
        GeoPathIterator pi = geoPath.getIterator(); // !!! ??? this.bezierConversionTolerance);
    ...
    private void writeRectangle(Rectangle2D rect) {
        final short x = (short)this.xToPagePx((float)rect.getMinX());
        final short y = (short)this.yToPagePx((float)rect.getMaxY());
        final short w = (short)this.dimToPagePx((float)rect.getWidth());
        final short h = (short)this.dimToPagePx((float)rect.getHeight());
        this.writeRectangleRecord(x, y, w, h);
    }
    
    private void writeCircle(double cx, double cy, double radius) {
        final short x = (short)this.xToPagePx((float)cx);
        final short y = (short)this.yToPagePx((float)cy);
        final short r = (short)this.dimToPagePx((float)radius);
    ...
    private void writeHeader() {
        ...
        final short w = (short)this.dimToPagePx((float)wWC);
        final short h = (short)this.dimToPagePx((float)hWC);
    ...
    private void writePenRecord(VectorSymbol symbol) {
        ...
        // stroke width
        final float w = symbol.getScaledStrokeWidth(this.getDisplayMapScale());
        short strokeWidth = (short)this.dimToPagePx(w);
        if (strokeWidth <= 0)
            strokeWidth = 1;
