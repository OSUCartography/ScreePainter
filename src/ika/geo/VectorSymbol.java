...
/**
 * VectorSymbol - graphic attributes of vector path. Used to draw the vector.
...
public class VectorSymbol extends Symbol implements java.io.Serializable, Cloneable {
    ...
    /**
     * The color used to draw the outter border of a vector path.
     */
    protected Color strokeColor = Color.black;
    
    /**
     * The width used to draw the outter border of a vector path.
     */
    ...
    /**
     * True: the outter border of a vector is drawn with the storkeColor. False: the
     * outter line is not drawn.
     */
    ...
    */
    ...
    public final float getScaledStrokeWidth(double scale) {
        float w = this.scaleInvariant ? 
            (float)(this.strokeWidth / scale) :
            this.strokeWidth;
        return w < 0f ? 0f : w;
    }
    ...
    public final Stroke getStroke(double scale) {
        ...
        if (dashLength <= 0f) {
            ...
            final float dashLength = this.getScaledDashLength(scale);
            stroke = new BasicStroke(scaledStrokeWidth,
                    this.cap,
                    BasicStroke.JOIN_BEVEL,
                    0,
                    new float[] {this.dashLength, this.dashLength},
                    0);
        }
