...
public class GeoImage extends AbstractRaster implements Serializable {
    ...
    throws IOException {
        ...
        if (this.image != null)
            ; // javax.imageio.ImageIO.write(this.image, "png", stream); this is called for undoing! it takes too much time for large images !!! ???
    }
    ...
    throws IOException, ClassNotFoundException {
        ...
        if (this.image == null)
            ...
            System.err.println("reading of GeoImage from file path not implemented yet"); // !!! ???
            this.image = null;
        }
        /* !!! ???
    ...
    public void drawNormalState(RenderParams rp) {
        if (this.image == null)
            return;
        this.drawImage(this.image, rp.g2d, rp.scale, 1);
    }
    ...
    protected void drawImage(BufferedImage image, Graphics2D g2d, 
            double mapScale, double imageScale) {
        
        //NanoTimer timer = new NanoTimer();
        //long start = timer.nanoTime();
        Rectangle2D.Double bounds = (Rectangle2D.Double)this.getBounds2D(mapScale);
        if (bounds == null)
            return;
        
        AffineTransform trans = new AffineTransform();
        trans.scale(1, -1);
        trans.translate(this.getWest(), -this.getNorth());
        trans.scale(bounds.getWidth() / this.image.getWidth() * imageScale,
                bounds.getHeight() / this.image.getHeight() * imageScale);
        g2d.drawImage(image, trans, null);
        //long end = timer.nanoTime();
        //System.out.println("Drawing: " + (end - start) / 1000 / 1000 + "ms");
    ...
    public final int getNearestGrayNeighbor(double x, double y) {
        // round to nearest neighbor
        int col = (int) Math.round((x - this.west) / this.cellSize);
        int row = (int) Math.round((this.north - y) / this.cellSize);
     ...
     */
    private static final int bilinearInterpolation(int c1, int c2, int c3, int c4,
