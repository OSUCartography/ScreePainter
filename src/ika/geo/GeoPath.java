...
public class GeoPath extends GeoObject implements Serializable, Cloneable{
    ...
    public void flatten(double flatness) {
        this.path = path.toFlattenedPath(flatness);
        MapEventTrigger.inform(this);
    }
     ...
     * the new straight lines approximating the bezier curve.
     */
    public GeoPath toFlattenedPath (double flatness) {
        GeoPath geoPath = (GeoPath)this.clone();
        geoPath.setPathModel(this.path.toFlattenedPath(flatness));
        return geoPath;
    }
    ...
    /**
     * This is a fix for a bug in the virtual machines on Windows and
     * Linux where bezier curves are not drawn if they are small.
     * It is not clear when this bug appears: which VM versions?, which
     * operating system versions? with what size of bezier curves? does
     * the bug depend on the map scale (or AffineTransform)? etc.
     * for now this is a very unelegant hack to temporarily solve this
     * problem.
     * Bezier curves are simply converted to flat lines with a maximum
     * flatness of 0.5 pixels.
     */
    private GeneralPath toDrawablePath(double scale) {
        return /*(ika.utils.Sys.isMacOSX())
        ? path.toGeneralPath()
        : */path.toFlattenedPath(0.5/scale).toGeneralPath();
    }
    ...
    public void drawNormalState(RenderParams rp) {
        ...
        final GeneralPath flattenedPath = this.toDrawablePath(scale);
    ...
    public void drawSelectedState(RenderParams rp) {
        ...
        // only stroke, no fill
        final GeneralPath flattenedPath = this.toDrawablePath(rp.scale);
    ...
    public Area toArea() {
        return new Area(this.path.toGeneralPath());
    }
