...
public class GridSlopeLineOperator implements GridOperator {
    ...
    private int nextUp(GeoGrid elevationGrid, GeoGrid counterGrid,
        ...
        for (int j = -1; j <= 1; j++) {
            for (int i = -1; i <= 1; i++) {
                float h = elevationGrid.getValue(currentCol + j, currentRow + i);
                float diff = h - hCenter;
                if (diff > maxDiff && diff >= this.getMinVerDiff()) {
                    maxDiff = diff;
                    r = i;
                    c = j;
                }
        ...
        int nextRow = currentRow + r;
        int nextCol = currentCol + c; 
        */
