...
public class GridFalllineOperator implements GridOperator {
    ...
    public enum SearchMethod {
        ...
        UP, DOWN, UP_THEN_DOWN, THROUGH_POINT
    };
     ...
     * the refGrid that already contains a reference to another object. refGrid
     * can be null. The found fall line does not have a width, i.e. line ends
     * behave like butt caps.
     * @param clipArea
     * @param curvatureGrid
     * @param minCurvature
     * @return
    ...
    public WeightedGeoPath operate(GeoGrid elevationGrid,
        ...
        switch (this.searchMethod) {
            case THROUGH_POINT: {
                ...
                path = extractSlopeLine(elevationGrid, startX, startY, true, binGrid, clipArea, curvatureGrid, minCurvature, mask255);
                if (path == null) {
                    path = extractSlopeLine(elevationGrid, startX, startY, false, binGrid, clipArea, curvatureGrid, minCurvature, mask255);
                } else {
                    path.invertDirection();
                    path.append(extractSlopeLine(elevationGrid, startX, startY, false, binGrid, clipArea, curvatureGrid, minCurvature, mask255), true);
                }
            ...
            case UP_THEN_DOWN: {
                ...
                path = extractSlopeLine(elevationGrid, endPoint.getX(), endPoint.getY(), true, binGrid, clipArea, curvatureGrid, minCurvature, mask255);
            }
            break;
     ...
     * @param pt On entry, contains the start point. On exit, contains the new
     * point.
     * @param downwards If true, search in downward direction, upwards
     * otherwise.
     ...
     * @param refGrid A grid with references to obstacles that the slope line
     * must not touch. refGrid can be null.
     * @param clipPolygon The slope line must be inside this polygon.
     * clipPolygon can be null.
     * @param weightGrid A grid containing an attribute that is accumulated
     * along the slope line. Important: The accumulated attribute is only a
     * rough guess, as the cell values of the weightGrid are not weighted with
     * the length of the corresponding line segments. weightGrid can be null.
     * @param minWeight Stop searching for the slope line if the value in
     * weightGrid is smaller than minWeight. Ignored if weightGrid is null.
    ...
    private WeightedGeoPath extractSlopeLine(GeoGrid elevationGrid,
        ...
        while (gridBounds.contains(x, y)) {
            ...
            // add point to line
            polyLine.moveOrLineTo((float) x, (float) y);
